...
def importar_relacion_carros(ws, conn):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        try:
            ...
            existe = conn.execute(
                "SELECT id FROM registros WHERE numero_economico=?",
                (str(folio),)
            ).fetchone()
            ...
            conn.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?)
            """, (nombre.title(), "", "", auto, str(folio), placas,
            ...
            if refacc > 0:
                ...
                try:
                    ...
                    """, ("Egreso", f"Refacciones Folio {folio} - {auto}",
                          refacc, fecha, servicio, hash_ref))
                except: pass

        except Exception as ex:
            errores += 1
            error("ImportadorContador", f"Error fila: {ex}")
